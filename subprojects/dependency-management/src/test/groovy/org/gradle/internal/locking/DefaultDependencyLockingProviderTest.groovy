...
class DefaultDependencyLockingProviderTest extends Specification {
    ...
    def 'can persist resolved modules as lockfile'() {
        ...
        then:
        lockDir.file('conf.lockfile').text == """${LockFileReaderWriter.LOCKFILE_HEADER_LIST.join(System.lineSeparator())}
org:bar:1.3
org:foo:1.0
"""
    }
    ...
    def 'can persist resolved modules as unique lockfile'() {
        ...
        then:
        lockDir.file(LockFileReaderWriter.UNIQUE_LOCKFILE_NAME).text == """${LockFileReaderWriter.LOCKFILE_HEADER_LIST.join(System.lineSeparator())}
org:bar:1.3=conf
org:foo:1.0=conf
empty=
"""
    ...
    def writeLockFile(List<String> modules, boolean unique = true, String configuration = 'conf') {
        if (unique) {
            lockDir.file(LockFileReaderWriter.UNIQUE_LOCKFILE_NAME) << """${LockFileReaderWriter.LOCKFILE_HEADER_LIST.join(System.lineSeparator())}
${modules.toSorted().collect {"$it=$configuration"}.join(System.lineSeparator())}
empty=
"""
        } else {
            lockDir.file("${configuration}.lockfile") << """${LockFileReaderWriter.LOCKFILE_HEADER_LIST.join(System.lineSeparator())}
${modules.toSorted().join(System.lineSeparator())}
"""
        }
    }
